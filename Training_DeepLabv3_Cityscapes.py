...
net.to(device)
print('pre-trained weight ADE20K is loaded')
print('====================')
        ...
        for phase in ['train', 'val']:
            ...
            for imges, anno_class_imges, mask_0_or_1 in tqdm(dataloaders_dict[phase]):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    # criterion=F.cross_entropy(outputs[0], targets, reduction='mean')
                    outputs = net(imges) 
                    ...
                    if phase == 'train':
                        train_acc += torch.sum(predicts == labels).item() / len(labels) / (labels.shape[1]*labels.shape[2]) # 475*475
